...
checksum_secret_input_set() {
  ...
  # Token names referenced by the template - default Kaptain shell-style
  # tokens are ${TOKEN_NAME}. Refine the regex if other delimiter/name
  # styles need to be supported in this code path. LC_ALL=C: the token
  # order feeds the hash input, so collation MUST be identical across
  # platforms and locales.
  local tokens
  tokens=$(grep -oE '\$\{[A-Za-z_][A-Za-z0-9_]*\}' "${template_file}" \
             | LC_ALL=C sort -u | sed -E 's/^\$\{|\}$//g')
  ...
  while IFS= read -r tok; do
    [[ -z "${tok}" ]] && continue
    # Encrypted values carry their encryption-type extension (<Token>.age
    # etc, per kaptain-user-scripts' kaptain-encrypt). First match in
    # LC_ALL=C order keeps this deterministic if more than one exists.
    value_file=$(compgen -G "${secrets_dir}/${tok}.*" | LC_ALL=C sort | head -1 || true)
